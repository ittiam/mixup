...
function parseBundle(bundleContent) {
  ...
  walk.recursive(ast, walkState, {
    CallExpression(node, state, c) {
      ...
      if (
        ...
        state.locations = getModulesLocationFromFunctionArgument(
          args[0].elements[1]
        );
        return;
      }
  ...
  return {
    src: bundleContent,
    modules: mapValues(walkState.locations, loc =>
      bundleContent.slice(loc.start, loc.end)
    ),
  };
}
...
function isArgumentContainsModulesList(arg) {
  if (arg.type === 'ObjectExpression') {
    return arg.properties.map(prop => prop.value).every(isModuleWrapper);
  }

  if (arg.type === 'ArrayExpression') {
    // Modules are contained in array.
    // Array indexes are module ids
    return arg.elements.every(
      elem =>
